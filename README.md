...
[![Skills](https://img.shields.io/badge/skills-120-blue)](#the-catalogue)
...
Full transcript: [`skills/reasoning/grill-me/examples/example.md`](skills/reasoning/grill-me/examples/example.md). 119 more skills below.
...
120 skills across 9 categories.
...
| [Second Order Effects Analyzer](skills/reasoning/second-order-effects-analyzer/) | Analyzes downstream cascade effects, systemic feedback loops, and unintended consequences of technical decisions. |
| [Socratic Interrogator](skills/reasoning/socratic-interrogator/) | Probes ambiguous specifications, hidden assumptions, and underspecified product requests through structured inquiry. |
...
| [Legacy Code Refactorer](skills/development/legacy-code-refactorer/) | Formulates safe Mikado Method refactoring dependency graphs and Strangler Fig extraction paths for fragile legacy codebases. |
...
| [Resilience Circuit Breaker Designer](skills/development/resilience-circuit-breaker-designer/) | Designs circuit breakers, bulkhead concurrency limits, fallback caches, and timeout policies for failing dependencies. |
...
| [Websocket Protocol Designer](skills/development/websocket-protocol-designer/) | Designs robust real-time bidirectional WebSocket/SSE protocols with heartbeat ping/pong, reconnect backoff, and auth. |
...
| [Dashboard Widget Designer](skills/ui-ux/dashboard-widget-designer/) | Designs high-signal analytics widgets, sparklines, KPI summary cards, and comparative metric badges with zero chartjunk. |
...
| [Form Ux Optimizer](skills/ui-ux/form-ux-optimizer/) | Optimizes complex multi-field web forms with inline validation timing, floating labels, accessible error lists, and smart auto-fill. |
...
| [Executive Summary Writer](skills/documentation/executive-summary-writer/) | Distills dense technical postmortems, architecture RFCs, and quarterly engineering metrics into crisp C-suite briefs. |
...
| [Sdk Getting Started Writer](skills/documentation/sdk-getting-started-writer/) | Crafts developer quickstart guides, language SDK installation snippets, and copy-pasteable curl/Python/TypeScript samples. |
...
| [Fuzz Testing Designer](skills/testing/fuzz-testing-designer/) | Creates property-based and mutation fuzz testing harnesses (AFL, Atheris, fast-check) to discover unexpected panics and crashes. |
...
| [Snapshot Testing Auditor](skills/testing/snapshot-testing-auditor/) | Audits brittle UI and JSON snapshot tests to eliminate false alarm failures and replace them with resilient semantic assertions. |
...
| [Cqrs Event Sourcing Designer](skills/architecture/cqrs-event-sourcing-designer/) | Designs Command Query Responsibility Segregation (CQRS) and Event Sourcing models with projection rebuilders. |
...
| [Feature Flag Lifecycle Manager](skills/architecture/feature-flag-lifecycle-manager/) | Establishes flag naming, fallback defaults, rollout rings, cleanup telemetry, and automated dead flag retirement. |
...
| [Multi Tenant Data Partitioner](skills/architecture/multi-tenant-data-partitioner/) | Architects database multi-tenancy models (Shared DB/Schema vs Separate Schema vs Separate Database) and tenant routing. |
...
| [Jwt Security Hardener](skills/security/jwt-security-hardener/) | Audits JSON Web Token implementations for algorithm confusion attacks, key rotation, expiration, and replay defense.', |
...
| [Soc2 Evidence Collector](skills/security/soc2-evidence-collector/) | Prepares automated audit trails, access review logs, change management records, and infrastructure evidence for SOC 2 audits. |
...
| [Git Conflict Resolver](skills/productivity/git-conflict-resolver/) | Resolves complex git rebase and merge conflicts with AST-level context, preserving intent across concurrent branches. |
...
| [Tech Debt Burndown Planner](skills/productivity/tech-debt-burndown-planner/) | Quantifies technical debt into an actionable, prioritized burndown backlog with clear ROI estimates. |
...
| [Git Hook Automation Designer](skills/utilities/git-hook-automation-designer/) | Configures pre-commit and pre-push hooks (Husky, Lefthook) for fast lint-staged, branch naming, and secret blocking. |
...
| [Otel Collector Configurator](skills/utilities/otel-collector-configurator/) | Designs OpenTelemetry Collector pipelines (receivers, processors, exporters) with batching and redaction. |
